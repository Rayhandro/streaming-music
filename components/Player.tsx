...
export function Player() {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-implicit-dependency
    setLyrics(null);
  ...
  const onReady = useCallback(async (event: any) => {
    ...
    // iOS Safari fix: auto-play when player becomes ready if isPlaying is true
    if (usePlayerStore.getState().isPlaying) {
      try {
        await event.target.playVideo();
      } catch {
        // iOS may block playVideo() without user gesture - handled by the play/pause sync effect
      }
    }
  }, [setDuration]);
  ...
  const onStateChange = useCallback(async (event: any) => {
    if (event.data === YouTube.PlayerState.PLAYING) {
      setPlaying(true);
      setNeedsManualPlay(false); // Hide manual play prompt once playback starts
      const duration = await event.target.getDuration();
      setDuration(duration || 0);
  ...
  useEffect(() => {
    if (playerRef.current) {
      if (isPlaying) {
        ...
        // If playVideo returns a promise (some browsers), catch rejection
        ...
        // Set a timeout to detect if playback didn't start (iOS Safari case)
        if (!playAttemptedRef.current) {
          playAttemptedRef.current = true;
          setTimeout(() => {
            if (playerRef.current && typeof playerRef.current.getPlayerState === 'function') {
              const state = playerRef.current.getPlayerState();
              // If not playing or buffering after 2 seconds, show manual play prompt
  ...
  return (
    <>
      ...
      {/* iOS Manual Play Prompt - shown when autoplay is blocked */}
      <AnimatePresence>
        {needsManualPlay && !isExpanded && (
          <motion.div
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-white/70 text-xs truncate">{currentTrack.name} - {artistName}</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
